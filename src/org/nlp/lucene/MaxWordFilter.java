...
public class MaxWordFilter extends TokenFilter {
	...
	private boolean hasIllegalOffsets; // only if the length changed before this
										// filter
	private MaxWordImpl wordSegmenter;
	private List<TokendWords> tokenBuffer;
	private Iterator<TokendWords> tokenIter;

	protected MaxWordFilter(TokenStream input) {
		super(input);
		wordSegmenter = new MaxWordImpl();
		MaxWordImpl.init("SogouLabDic.dic", true);
	}
